...
void prevent_main(int *func, USER *u)
{
  ...
  int num = 0;
  int flag = 0;
  int state_city = 0; //用来记录中高风险地区展开状况
  int state = 0;      //来记录标语翻页状态
  int hour1, min1;
  ...
  while (1)
  {
    ...
    if (MouseX > 230 && MouseX < 270 && MouseY > 230 && MouseY < 260)
    {
      ...
      {
        if (state_city == 0) //表示未展开
        {
          city_read(c);
          get_cityinfo(c);
          delay(100);
        }
        if (state_city == 1) //表示已展开
        {
...
void prevent_draw(void)
{
  ...
  puthz(85, 410, "疫情期间，请注意防护！", 16, 18, 6);
}
