...
int MouseX;  //鼠标位置x
int MouseY;  //鼠标位置y
int MouseS;  //鼠标形状Shape 1为手势，2为光标，3为十字
int press;   //鼠标按压状态 1为左键，2为右键，3为未按下，其他为默认鼠标
void *buffer;
union REGS regs;
int flag=0;  //鼠标画面存在状态
...
/***************************************
FUNCTION: mread
DESCRIPTION: 获取新的寄存器信息，获取鼠标位置和按键状态，在newmouse中调用改变MOUSEX和MOUSY以及press数值
...
/*******************************************
FUNCTION: newmouse
DESCRIPTION: 鼠标状态发生变化则更新鼠标，未变化则直接返回
INPUT: nx,ny,nbuttons
RETURN: 无
********************************************/
void newmouse(int *nx,int *ny,int *nbuttons)
{
	int xn,yn,buttonsn;//记录鼠标状态量
	int x0=*nx,y0=*ny,buttons0=*nbuttons;
